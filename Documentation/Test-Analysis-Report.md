...
Cursor was used **after** that plan existed, mainly to follow the flow I had already set: install and wire Vitest, RTL, MSW, and ESLint,E2E; add tests file by file according to the assignment; and draft documentation from my test results. The AI suggested code and wording, but I reviewed every change in the editor—especially Swedish validation messages, mock payloads, and which behaviours each test actually covers. I run `npm test` , `npm run lint`,`npm run test:e2e` and `npm run test:e2e:ui` after changes, and I make all git commits locally (with conventional commit messages I choose).
